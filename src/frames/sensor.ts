...
export type Region = { id: string; rects: Rect[]; pixelRects: Rect[] };
export type Frame = {
  kind: string;
  note: string;
  dataUrl: string;
...
export function textBox(
  ...
  txt(x, s, X, Y, size, color, weight, align);
  const m = x.measureText(s);
  const l = Math.floor(X - m.actualBoundingBoxLeft),
    t = Math.floor(Y - m.actualBoundingBoxAscent),
    r = Math.ceil(X + m.actualBoundingBoxRight),
...
export function subtract(a: Rect, b: Rect): Rect[] {
  ...
  if (l >= r || t >= d) return [a];
  return [
    { x: a.x, y: a.y, w: a.w, h: t - a.y },
    { x: a.x, y: d, w: a.w, h: a.y + a.h - d },
    { x: a.x, y: t, w: l - a.x, h: d - t },
...
export function region(id: string, boxes: Rect | Rect[], W: number, H: number): Region {
  ...
  return {
    id,
    rects: arr.map((b) => ({ x: b.x / W, y: b.y / H, w: b.w / W, h: b.h / H })),
    pixelRects: arr,
  };
}
...
export function finish(
  ...
  kind: string,
  note: string,
  targets: [string, Rect | Rect[]][],
  seals: [string, Rect | Rect[]][],
): Frame {
  const W = c.width,
    H = c.height;
  return {
    ...
    W,
    H,
    targets: targets.map(([id, b]) => region(id, b, W, H)),
...
export function grain(x: Ctx, W: number, H: number, amount: number) {
  ...
  for (let i = 0; i < d.length; i += 4) {
    const luminance = (d[i] + d[i + 1] + d[i + 2]) / 765;
    const n = (Math.random() + Math.random() - 1) * amount * (1 - 0.45 * luminance);
    d[i] += n;
    d[i + 1] += n * 0.9;
    d[i + 2] += n * 1.08;
  }
...
export function sensor(
  ...
  const W = x.canvas.width,
    H = x.canvas.height,
    [small, s] = mk(Math.round(W * scale), Math.round(H * scale));
  s.drawImage(x.canvas, 0, 0, small.width, small.height);
  x.save();
  x.imageSmoothingEnabled = true;
  x.drawImage(small, 0, 0, W, H);
  x.restore();
  const [copy, k] = mk(W, H);
  k.drawImage(x.canvas, 0, 0);
  x.save();
  x.globalCompositeOperation = 'screen';
  x.globalAlpha = bleed;
  x.filter = 'blur(2px)';
  x.drawImage(copy, 2, 0);
  x.filter = 'blur(7px)';
  x.globalAlpha = 0.075;
  ...
  for (let yy = 0; yy < H; yy += 16)
    for (let xx = 0; xx < W; xx += 16) {
      const v = rr();
      if (v < 0.14)
        rect(x, xx, yy, 16, 16, v < 0.07 ? 'rgba(0,0,0,.018)' : 'rgba(185,202,206,.012)');
    }
  if (tear) {
    for (const yy of [Math.round(H * 0.32), Math.round(H * 0.71)]) {
      x.save();
      x.globalAlpha = 0.16;
      x.drawImage(copy, 0, yy, W, 2, 3, yy, W, 2);
      x.restore();
    }
  }
  if (bleed) {
    const im = x.getImageData(0, 0, W, H),
      p = im.data,
      src = new Uint8ClampedArray(p);
    const mix = Math.min(0.48, bleed * 5);
    for (let y = 0; y < H; y++)
      for (let xx = 2; xx < W - 2; xx++) {
        const i = (y * W + xx) * 4;
        p[i] = src[i] * (1 - mix) + src[i - 8] * mix;
        p[i + 2] = src[i + 2] * (1 - mix) + src[i + 6] * mix;
      }
    x.putImageData(im, 0, 0);
  }
  grain(x, W, H, noise);
  if (scan) for (let y = 0; y < H; y += 3) rect(x, 0, y, W, 1, `rgba(0,0,0,${scan})`);
  vignette(x, W, H, vignettePower);
}
